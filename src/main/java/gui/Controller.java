...
// The begin of StudentsTab's methods ------------------------------------------------------
    ...
    public void handleDeleteButtonFromStudentsTab(ActionEvent actionEvent) {
        if (tableFromStudentsTab.getSelectionModel().getSelectedItem() != null){
            ...
            MessageAlert.showMessage(null, Alert.AlertType.CONFIRMATION, "The student has been deleted!","The student has been deleted!");
    ...
    private void  showEditStudentDialog(){
        try{
            // create a new stage for the popup dialog.
            FXMLLoader loader = new FXMLLoader();
            loader.setLocation(getClass().getResource("/views/editStudent.fxml"));
            ...
            // Create the dialog Stage.
            Stage dialogStage = new Stage();
            dialogStage.setTitle("Preview Student");
            dialogStage.initModality(Modality.WINDOW_MODAL);
            //dialogStage.initOwner(primaryStage);
            Scene scene = new Scene(root);
            dialogStage.setScene(scene);
            ...
            editStudentController.setService(
                    dialogStage, this.studentService, nameTextFieldFromStudentsTab.getText(),
                    firstNameTextFieldFromStudentsTab.getText(),
                    emailTextFieldFromStudentsTab.getText(),
                    groupChoiceBoxFromStudentsTab.getValue(),
                    teacherIdTextFieldFromStudentsTab.getText()
    ...
    public void handleAddButtonFromStudentsTab(ActionEvent actionEvent) {
        ...
        this.clearFieldsFromStudentsTab();
    }
...
// The begin of TeachersTab's methods ------------------------------------------------------
    ...
    private void showEditTeacherDialog(){
        ...
        try {
            // create a new stage for the popup dialog.
            FXMLLoader loader = new FXMLLoader();
            loader.setLocation(getClass().getResource("/views/editTeacher.fxml"));

            AnchorPane root = (AnchorPane) loader.load();

            // Create the dialog Stage.
            Stage dialogStage = new Stage();
            dialogStage.setTitle("Preview Teacher");
            dialogStage.initModality(Modality.WINDOW_MODAL);
            //dialogStage.initOwner(primaryStage);
            Scene scene = new Scene(root);
            dialogStage.setScene(scene);
            ...
            editTeacherController.setService(
                    dialogStage,
                    this.teacherService,
...
//        }
        this.teacherService.getMessagesOnPage(this.teacherService.getPage()).forEach(teachersList::add);
...
//                initHomeworkModel();
    ...
    @FXML
    public void handleNextPageButtonFromTeachersTab(ActionEvent actionEvent) {
        int nr = Integer.parseInt(pageTextFieldFromTeachersTab.getText());
        nr++;
        if(!noOfPagesMaxTeachersTab()) {
            pageTextFieldFromTeachersTab.setText(
                    String.valueOf(nr)
            );
            simplifiedHandlePageTextFieldFromTeachersTab();
        }
    }

    @FXML
    public void handlePageTextFieldFromTeachersTab(KeyEvent keyEvent) {
        simplifiedHandlePageTextFieldFromTeachersTab();
    }

    @FXML
    public void handlePageCheckBoxFromTeachersTab(ActionEvent actionEvent) {
        if (pageCheckBoxFromTeachersTab.isSelected())
            pageTextFieldFromTeachersTab.setEditable(true);
        else
            pageTextFieldFromTeachersTab.setEditable(false);
    }

    @FXML
    public void handlePreviousPageButtonFromTeachersTab(ActionEvent actionEvent) {
        int nr = Integer.parseInt(pageTextFieldFromTeachersTab.getText());
        if(nr  > 0){
            nr--;
            pageTextFieldFromTeachersTab.setText(
